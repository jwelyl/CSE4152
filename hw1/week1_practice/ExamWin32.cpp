...
int main() {
	...
	int inner_circle = 0, outter_circle = 0;
	float calPi = 0;

	// TODO

	for (int i = 0; i < MAX_POINTS; i++) {
		float x = (float)(distribution(engine) - 10000) / 100;
		float y = (float)(distribution(engine) - 10000) / 100;

		if (x * x + y * y <= 10000) {
			Ellipse(CENTER_X + x, CENTER_Y + y, CENTER_X + x + POINTS_RADIUS, CENTER_Y + y + POINTS_RADIUS, RGB(47, 208, 219), RGB(47, 208, 219));
			inner_circle++;
		}
		else {
			Ellipse(CENTER_X + x, CENTER_Y + y, CENTER_X + x + POINTS_RADIUS, CENTER_Y + y + POINTS_RADIUS, RGB(212, 208, 94), RGB(212, 208, 94));
			outter_circle++;
		}
	}

	calPi = (float)inner_circle / (inner_circle + outter_circle) * 4;
