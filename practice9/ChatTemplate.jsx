...
function ChatTemplate() {
	...
	const getChatLog = () => {
		...
		// 9주차 실습 구현
		console.log("현재 chats :", chats);

		axios.get("http://localhost:8001/get")
			.then((res) => {
				if(chats.length < res.data.length) {
					setChats(chats.concat(res.data[chats.length]));
					setChatLen(chats.length + 1);
				}
			});
	}
	...
	const onSearchKeySubmit = (keyword) => {
		// 9주차 실습 구현
		console.log("keyword in onSearchKeySubmit :", keyword);
		console.log("chats in onSearchKeySubmit :", chats);

		for(let i = 0; i < chats.length; i++) {
			if(chats[i].message.includes(keyword) === true) {
				// setChats({message: chats[i].message, username: chats[i].username, searched: true});
				chats[i].searched = true;
				// setChats([...chats]);
				console.log("search후 갱신된 chats: :", chats);
			}
		}
		setChats([...chats]);


		console.log("getChatLog in onSearchKeySubmit");
